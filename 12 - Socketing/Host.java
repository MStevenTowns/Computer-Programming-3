...
public class Host
{
    public static void main(String[] args) throws Exception
    {
		...
		TicTacToeGrid board=new TicTacToeGrid();
		boolean go=true;
		int x,y,outcome;
		String run;
		outcome=0;
        while(!board.isWin(11) && !board.isWin(10) && !board.isCat())
        {
            System.out.println("Your move");
            System.out.println(board.print());
            System.out.println("Where do you want to go?");
            int choice=Integer.parseInt(keyRead.readLine());
            board.play(board.convertChoice(choice)[0],board.convertChoice(choice)[1],11);
            String out=board.print();
            if(board.isWin(11))
            {
                System.out.println("You Win");
                pwrite.println("Opponent Wins");
            }
            pwrite.println(out);             
            pwrite.flush();
            System.out.println("Opponent's move");
            choice = Integer.parseInt(receiveRead.readLine());
            board.play(board.convertChoice(choice)[0],board.convertChoice(choice)[1],10);
        } 
        
        if(board.isWin(10))
        {
            System.out.println("Opponent Wins");
            pwrite.println("You Win");
        }
        else 
        {
            System.out.println("No winner");
            pwrite.println("No Winner");
        }
    }     
    static class TicTacToeGrid 
    {
